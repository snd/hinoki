...
(function(root, factory) {
  ...
  var hinoki;
  hinoki = function(oneOrManyContainers, nameOrNamesOrFunction, debug) {
    var containers, path, paths;
    ...
    if ('function' === typeof nameOrNamesOrFunction) {
      return hinoki(oneOrManyContainers, hinoki.getNamesToInject(nameOrNamesOrFunction).map(hinoki.coerceToArray), debug).spread(nameOrNamesOrFunction);
    }
    ...
    return hinoki.one(containers, path, debug);
  };
  hinoki.many = function(containers, paths, debug) {
    return Promise.all(paths.map(function(path) {
      return hinoki.one(containers, path, debug);
    }));
  };
  hinoki.one = function(containers, path, debug) {
    var base, dependenciesPromise, dependencyNames, dependencyPaths, error, factoryCallResultPromise, newPath, nocache, promiseAwaitingResolution, ref, remainingContainers, resolution, resolutionErrors;
    try {
    ...
    dependenciesPromise = dependencyPaths.length !== 0 ? hinoki.many(remainingContainers, dependencyPaths, debug) : Promise.resolve([]);
