...
(function(root, factory) {
  ...
  hinoki.getValueAndCacheTarget = function(source, lifetimes, path, cacheTarget) {
    var dependenciesPromise, dependencyName, dependencyNames, dependencyNamesIndex, dependencyNamesLength, dependencyPaths, factory, factoryCallResultPromise, lifetimeIndex, name, newPath, nextCacheTarget, promise, result, returnPromise, valueOrPromise;
    name = path[0];
    ...
    while (++dependencyNamesIndex < dependencyNamesLength) {
      ...
      if (-1 !== path.indexOf(dependencyName)) {
  ...
  hinoki.callFactoryObjectArray = function(path, factoryObject, dependenciesObject) {
    ...
    if (Array.isArray(factoryObject)) {
      ...
      return Promise.props(_.mapValues(factoryObject, iterator));
    }
  };
