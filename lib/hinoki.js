...
(function(root, factory) {
  ...
  var hinoki;
  hinoki = function(oneOrManyLifetimes, nameOrNamesOrFunction, debug) {
    ...
    if ('function' === typeof nameOrNamesOrFunction) {
      return hinoki(oneOrManyLifetimes, hinoki.getNamesToInject(nameOrNamesOrFunction).map(hinoki.coerceToArray), debug).spread(nameOrNamesOrFunction);
    }
    ...
    return hinoki.one(lifetimes, path, debug);
  };
  hinoki.many = function(lifetimes, paths, debug) {
    return Promise.all(paths.map(function(path) {
      return hinoki.one(lifetimes, path, debug);
    }));
  };
  hinoki.one = function(lifetimes, path, debug) {
    var base, dependenciesPromise, dependencyNames, dependencyPaths, error, factoryCallResultPromise, newPath, nocache, promiseAwaitingResolution, ref, remainingLifetimes, resolution, resolutionErrors;
    try {
    ...
    dependenciesPromise = dependencyPaths.length !== 0 ? hinoki.many(remainingLifetimes, dependencyPaths, debug) : Promise.resolve([]);
    factoryCallResultPromise = dependenciesPromise.then(function(dependencyValues) {
      return hinoki.callFactory(resolution.lifetime, newPath, resolution.factory, dependencyValues, debug);
    });
    if (!nocache) {
      if ((base = resolution.lifetime).promisesAwaitingResolution == null) {
    ...
    return factoryCallResultPromise.then(function(value) {
      ...
      if (!nocache) {
        if ((base1 = resolution.lifetime).values == null) {
          base1.values = {};
        }
  ...
  hinoki.callFactory = function(lifetime, path, factory, dependencyValues, debug) {
  ...
  hinoki.resolveInLifetime = function(lifetime, path, debug) {
  ...
  hinoki.resolveInLifetimes = function(lifetimes, path, debug) {
    return hinoki.some(lifetimes, function(lifetime) {
      return hinoki.resolveInLifetime(lifetime, path, debug);
    });
  };
