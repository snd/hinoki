...
(function(root, factory) {
  ...
  var hinoki, hinokiAndCacheNamesToInject, hinokiNamesToInject, hinokiOne;
  hinoki = function(oneOrManyContainers, oneOrManyNamesOrPaths, debug) {
    var containers, injectable;
    ...
    if (Array.isArray(oneOrManyNamesOrPaths)) {
      return Promise.all(oneOrManyNamesOrPaths.map(function(nameOrPath) {
        return hinokiOne(containers, nameOrPath, debug);
      }));
    } else if ('function' === typeof oneOrManyNamesOrPaths) {
      injectable = oneOrManyNamesOrPaths;
      return hinoki(oneOrManyContainers, hinokiNamesToInject(injectable), debug).spread(injectable);
    } else {
      return hinokiOne(containers, oneOrManyNamesOrPaths, debug);
    }
  };
  hinokiOne = function(containers, nameOrPath, debug) {
    ...
    dependenciesPromise = dependencyPaths.length !== 0 ? hinoki(remainingContainers, dependencyPaths, debug) : Promise.resolve([]);
  ...
  hinokiNamesToInject = function(factory) {
  ...
  hinokiAndCacheNamesToInject = function(factory) {
    var names;
    if (factory.$inject != null) {
      return factory.$inject;
    } else {
